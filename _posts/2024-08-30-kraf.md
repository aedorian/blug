...
A `Chunk::generateChunk()` function fills the block data using noises. Here's a first test:
...
<div class="sideside" markdown="1">
Here's a tree! But there's a problem... Since it must have been placed on the side of a chunk, half of it got cut. To fix this, we just have to tell the `setBlock(x, y, z)` function to place the block in a neighbouring chunk if the position is out of range.
{: .tleft }
<img src="../assets/img/kraf/cuttree.PNG" height=300px>
{: .imright }
</div>
