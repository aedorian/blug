...
I'm going to re-explain some concepts here, but if you're interested, check out their videos.
...
To fix this, I added a `Chunk *neighbours[4]` variable to the `Chunk` class. This way, when it checks if a face on the chunk's border is free, it can look into the neighbouring chunk's data.
...
It's finally time to get into world generation! I'm using the [FastNoise2 library](https://github.com/Auburn/FastNoise2) for everything noise-related. It's a node-based library and it even has a graphical editor software.

A `World` class is used to store a `ChunkManager`, a seed and noise parameters.
...
I decided to keep the terrain simple at first and went on to place trees (which are going to be defined as structures in the world).

I made a new structure to store a structure (yes) and a `Chunk::placeStructure(Structure s, glm::ivec3 pos)` function.
...
<div class="sideside" markdown="1">
I made a `World::placeStructure(Structure *s, Chunk *c)` function to place structures. And here's a tree! But there's a problem... Since it must have been placed on the side of a chunk, half of it got cut out. I used this method to fix it:
{: .tleft }
<img src="../assets/img/kraf/cuttree.PNG" height=250px>
{: .imright }
</div>

The idea is that when a block can't be placed by the `placeStructure()` function because its coordinates are out of bounds, the `World` object needs "cache" this block.

I added a `std::map<int, std::vector<CachedBlock>> cachedBlocks` variable in the `World` class, as well as a `World::getChunkHash(Chunk *c)` function that returns an `int` hash code for a `Chunk` using its position. A `CachedBlock` is a simple `struct` containing a block type and its position.
  
When a block can't be placed by the `placeStructure()` function because its coordinates are out of bounds, it tries placing it in a neighbouring chunk until it finds valid coordinates (using a recursive function).

The new block coordinates are added to the `cachedBlocks[getChunkHash(c)]` vector of `CachedBlocks`.

Finally, when a chunk is requested, we check in `cachedBlocks` if there are any cached blocks in this chunk. If so, we place the blocks and add the chunk back to `unbuildChunks` to recalculate its mesh.

<figure style="display: inline-block">
<img src="../assets/img/kraf/treeparts1.PNG" height=250px>
<figcaption>cached blocks in red</figcaption>
</figure>
<figure style="display: inline-block">
<img src="../assets/img/kraf/treeparts2.PNG" height=250px>
...
grass with two quads

block placing/breaking (raycast)
...
rebuilding is BAD? yes or no
it turns out recalculating the entire mesh isn't that expensive (the chunk is *only* 16*16*256). (plus, for placing blocks, we could afford not recalculating the mesh and just adding to it NO)
