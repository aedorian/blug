...
The [source code of this project]() is also available if you'd like to check it out.
...
Now when a block can't be placed in a chunk by `placeStructure()`, it finds the right neighbouring chunk `c` to place it in. The new `CachedBlock` is added to the `cachedBlocks[c.hash]` vector.
...
<div class="sideside" markdown="1">
The take-away here is that each vertex can have one of 4 AO values (between 0.0 and 1.0) calculated with the corner and side blocks. Then, in the fragment shader, the block's color is multiplied by the vertex's AO value, effectively darkening certain edges. Here's a visualization with disabled textures:
{: .tleft }
...
I also calculate a lighting value based on time, and send it to the block shader as a uniform. This makes the blocks darker at night.
...
I made a `Block inventory[9]` array, and made the currently selected block render to the corner of the screen. The player can switch blocks by scrolling the mouse.
...
make block placing
make inventory and callback
show currently selected block
make day night cycle

remove raycast block in main
REENABLE FACE CULLING AFTER DEBUGGING IN WINDOW.CPP!!!!!!!
